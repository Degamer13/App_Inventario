...
$verifica_serial=mysqli_query($conex, "SELECT * FROM mobiliarios WHERE serial_number='$serial_number'");
...
$stmt = $conexion->prepare("INSERT INTO mobiliarios (descripcion, serial_number, ubicacion, marca, cantidad, fecha)
 VALUES (?, ?, ?, ?, ?, ?)");
...
$dompdf = new Dompdf();
$dompdf->loadHtml('<h1>Entrada de Mobiliarios</h1><p>Descripción: '.$descripcion.'</p><p>Serial de Numero: '.$serial_number.'</p>
...
$file = 'entrada_mobiliarios_' . $fechaHora . '.pdf';
...
echo '<script>';
    ...
    echo '    Swal.fire({';
    echo '      icon: "success",';
    echo '      title: "Mobiliario registrado correctamente",';
